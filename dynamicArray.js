function dynamicArray(n, queries) {
    ...
    queries.forEach(q => {
        ...
        let formula = ((q[1]^lastAnswer) % n);
        console.log("***Formula: " + formula);

        if(q[0] === 1) // Query type 1
        {
            seqList[formula].push(q[2]);
        }
        else // Query type 2
        {
            let seq = seqList[formula];
            console.log("Sequence found: " + seq);
            let seqIndex = q[2]%seq.length;
            console.log("Seq index needed: " + seqIndex + " of seq.length: " + seq.length);
